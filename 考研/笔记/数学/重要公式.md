...
3. $\lim_{n\rarr\infty}\sqrt[n]{|x_1^n+x_2^n+...+x_m^n|}=max\{|x_1|,|x_2|,...|x_n|\}$
...
4. $\int_0^a\frac{1}{x^p}dx(a>0)\begin{cases}p<1:收敛\\p\ge1:发散\end{cases}$
   + $\sim\int_1^a\frac{1}{(x-1)^p}dx\sim\int_3^a\frac{1}{(x-3)^p}dx$
...
3. 对于正项级数$\sum a_n收敛\Rarr\sum a_n^2收敛$
