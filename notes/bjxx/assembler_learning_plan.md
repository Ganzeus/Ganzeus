...
想象一个 8×8 的格子gf，每个格子里有一个小计算器（PE）。这个计算器只会做一件事：**把两个数相乘，然后加到自己的累加器上**（MAC = Multiply-Accumulate）。
...
2. **激活值（输入数据）**从左侧逐行流入
3. 每个 PE 在每个时钟周期做一次 **MAC 运算**：`累加器 += 输入 × 权重`
4. 计算完一行后，**部分和（partial sum）**往下传给下一行 PE 继续累加
5. 最终从底部流出完整的输出
...
### VP（Vector Processor）—— 向量计算单元

| 属性 | 值 |
|---|---|
| 功能 | 元素级运算（Add/Mul/Div）、激活函数（ReLU/Sigmoid/GELU/Softmax）、池化（AvgPool/MaxPool）、归一化（LayerNorm/BatchNorm） |
| 特点 | 处理 PE 算完后需要做的"后处理" |

**对应指令**：VP_CFG_FM (192), VP_CFG_GCS (193), VP_SELF (200), VP_UPSM (201), VP_POOL (202) 等。

**大模型里 VP 极其重要**：Transformer 里的 Softmax、LayerNorm、GELU、残差 Add 全在 VP 上跑。
...
| L0 | OL1 | 36 B/core | 部分和（PE 累加的中间结果） | — | PE |
